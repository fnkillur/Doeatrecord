...
const Spending = ({userId, now}) => {
  const {loading, error, data} = useQuery(GET_SPENDING, {variables: {userId, now}});
  ...
  return (
    <article className="spending">
      <div className="spending-field">
        {now ? '이번 달 ' : '지금까지 '}
        <strong className="spending-strong">{convertMoney(total)}원</strong> 썼다!
